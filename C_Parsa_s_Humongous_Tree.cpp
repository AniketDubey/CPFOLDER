...
void dfs(ll u, ll par)
{
    for(ll x: arr[u])
    {
        ...
        {
            dfs(x, u);
            dp[u][0] += max(abs(vec[u - 1].ff - vec[x - 1].ff) + dp[x][0], abs(vec[u - 1].ff - vec[x - 1].ss) + dp[x][1]);
            dp[u][1] += max(abs(vec[u - 1].ss - vec[x - 1].ff) + dp[x][0], abs(vec[u - 1].ss - vec[x - 1].ss) + dp[x][1]);
        }
    }
}
