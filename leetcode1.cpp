...
int main()
{
    ...
    {
        freq.insert(rating[i]);
        auto pos = lower_bound(freq.begin(), freq.end(), rating[i]);

    } */

    /* auto pos = lower_bound(freq.begin(), freq.end(), 6);

    int index = pos - freq.begin();

    cout << *pos << endl; */

    /*auto pos = freq.find(56);

    auto pos1 = freq.begin();
