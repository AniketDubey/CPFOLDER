...
int main()
{
    ...
    w(t)
    {
        ...
        // if mid  = 5 and at that movement there is no winner so that means that there will not be any winner
        // before chance no. 5 and hence increase low = mid+1 as winner would be possible only after mid and
        // before high.............
        // Similarly if at mid = 10 we have a winner so decrease high = mid-1, and check from low to mid again
        ...
        while(low<=high)
        {
            ...
            // populate the matrix upto mid chances..........
            for0(i,mid+1)
            {
                if(i%2==0)
                    arr[vec[i].ff][vec[i].ss] = 1;
                else
                    arr[vec[i].ff][vec[i].ss] = -1;
            }

            // calculate prefix sum of the matrix formed upto mid chances...........
            //  here we have created a 2d prefix sum matrix 
            for1(i,n)
            {
                for1(j,m)
                {
                    // formula for 2d prefix sum.........
                    // prefix[i][j] = arr[i][j] + arr[i-1][j] + arr[i][j-1] - arr[i-1][j-1]
            ...
            for (ll i = k; i <= n;i++)
            {
                for (ll j = k; j <= m;j++)
                {
                    ll cnt = arr[i][j] - arr[i - k][j] - arr[i][j - k] + arr[i - k][j - k];
